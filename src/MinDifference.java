...
represents numbers on a tape. Any integer P, such that 0 < P < N, splits this tape 
into two non-empty parts: A[0], A[1], ..., A[P − 1] and A[P], A[P + 1], ..., A[N − 1].
...
public class MinDifference {
    ...
    public static int findMinDifference(int[] A) {
        ...
        for (int i = 0; i<A.length-1; i++){
            leftSum += A[i];
            rightSum -= A[i];
            int newDiff = Math.abs(rightSum -leftSum);
            if (newDiff == 0){
                minDiff = newDiff;
                break;
